...
class TrabajosLibres extends Controller
{
  ...
  public function saveTrabajo()
  {
    ...
    $titulo = MasterDom::getData('titulo');
    $autor = MasterDom::getData('autor');
    $email = MasterDom::getData('email');
    $grupo = MasterDom::getData('grupo');
    $no_poster = MasterDom::getData('no_poster');
    $pdf = MasterDom::getData('pdf');
    $clave = $this->generateRandomString(6);
    ...
    $data->_email = $email;
    $data->_grupo = $grupo;
    $data->_no_poster = $no_poster;
    $data->_pdf = $clave.".pdf";

    if (!file_exists('trabajos_files/')) {
      mkdir('trabajos_files/', 0777, true);
    }



    if (move_uploaded_file($pdf["tmp_name"], "trabajos_files/" . $clave . '.pdf')) {
      $id = TrabajosLibresDao::insert($data);
      if ($id) {
        echo 'success';
      } else {
        echo 'fail';
...
html;
  ...
  public function getAllCursos()
  {
    ...
    $html = "";
    foreach (TrabajosLibresDao::getAll() as $key => $value) {
            ...
            <tr>
                    ...
                    <div class="d-flex px-3 py-1">
                        ...
                        <div class="d-flex flex-column justify-content-center text-black">
                                ...
                                <h6 class="mb-0 text-sm text-move text-black">
                                    <p><span class=""></span> Grupo : {$value['grupo']}</p>
                                    <p><span class=""></span> No. Poster : {$value['no_poster']}</p>
                                    <p><span class=""></span> Titulo : {$value['titulo']}</p>
                                    <p><span class=""></span> Autor : {$value['autor']}</p>                                   
                ...
                <div class="d-flex flex-column justify-content-center text-black">                    
                        ...
                        <h6 class="mb-0 text-sm  text-black">
                            <p><a href="../trabajos_files/{$value['pdf']}" target="_blank">Documento</a> </p>                            
