...
sql;
  ...
  public static function getAllColaboradoresByName($search){
    $mysqli = Database::getInstance();
    $query =<<<sql
    SELECT re.id_registrado, re.nombre, re.apellidom, re.apellidop, re.email, re.activo,re.especialidad,re.prefijo,re.telefono,re.id_estado, re.id_pais,pa.pais, est.estado,re.clave, esp.nombre as nombre_especialidad
    FROM registrados re
    LEFT JOIN paises pa ON (re.id_pais = pa.id_pais)
    LEFT JOIN estados est ON (est.id_estado = re.id_estado)
    LEFT JOIN especialidades esp ON(esp.id_especialidad = re.especialidad)
